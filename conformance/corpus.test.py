...
class DefinitionTest(unittest.TestCase):
    ...
    def test_the_corpus_names_the_chip_its_audio_came_from(self):
        self.assertIn("snes9x", corpus.load()["reference"])

    def test_the_corpus_records_the_census_it_was_drawn_from(self):
...
class ShapeTest(unittest.TestCase):
    def test_every_case_carries_a_whole_register_file(self):
        wrong = [c for c in corpus.load()["cases"] if len(c["registers"]) != 128]

        self.assertEqual(wrong, [])

    def test_the_cases_use_many_different_sample_directories(self):
        pages = {c["registers"][0x5D] for c in corpus.load()["cases"]}

        self.assertGreater(len(pages), 20)

    def test_the_cases_reach_the_envelope_the_echo_and_the_noise(self):
    ...
    def test_the_corpus_holds_no_audio_data(self):
        self.assertEqual(
...
class RamTest(unittest.TestCase):
    def test_the_sample_data_is_rebuilt_from_its_seed(self):
        case = corpus.load()["cases"][0]

        self.assertEqual(corpus.ram_for(case), corpus.ram_for(case))

    def test_the_sample_data_fills_the_whole_space(self):
        self.assertEqual(len(corpus.ram_for(corpus.load()["cases"][0])), 0x10000)

    def test_a_source_number_that_runs_past_the_space_falls_back(self):
    ...
    def test_the_directory_points_where_the_real_registers_said(self):
        case = corpus.load()["cases"][0]
        ram = corpus.ram_for(case)
...
class CheckTest(unittest.TestCase):
    ...
    def test_a_case_that_cannot_run_is_reported_rather_than_raising(self):
