...
Sony published no data sheet for this part either. What settles it is the clock
schedule: the hardware does a fixed thirty two steps per sample, in a fixed
order, and this model does the same thirty two in the same order rather than
computing a sample and calling it done. A voice read that the hardware performs
on step 9 is performed on step 9 here, which is what makes a read of a register
mid sample answer what the hardware answers.

It is a part rather than a clocked part. It answers reads and writes of its
registers, and nothing a host does to it is measured in cycles: there is no
instruction to step through and no budget to hand back. The thirty two steps are
the part's own schedule rather than a caller's budget, so the section about a
clock does not apply and this repository carries none of the interface it
describes.
