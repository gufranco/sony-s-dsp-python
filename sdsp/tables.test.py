...
class GaussianTest(unittest.TestCase):
    def test_the_kernel_is_a_quarter_of_the_curve(self):
        self.assertEqual(len(tables.GAUSSIAN), 512)

    def test_the_kernel_rises_to_its_peak_at_the_end(self):
        self.assertEqual(tables.GAUSSIAN[-1], max(tables.GAUSSIAN))

    def test_the_kernel_starts_at_nothing(self):
        self.assertEqual(tables.GAUSSIAN[0], 0)

    def test_the_kernel_never_decreases(self):
        falling = [
            index
    ...
    def test_the_four_taps_of_any_position_sum_to_one_unit(self):
...
class CounterTest(unittest.TestCase):
    ...
    def test_the_offsets_repeat_a_short_stagger_pattern(self):
        self.assertEqual(set(tables.COUNTER_OFFSETS[1:]), {0, 536, 1040})

    def test_the_stagger_cycles_every_three_rates(self):
        middle = tables.COUNTER_OFFSETS[1:28]

        self.assertEqual(middle, tuple(middle[index % 3] for index in range(len(middle))))

    def test_an_offset_larger_than_its_rate_is_taken_modulo_it(self):
        wrapping = [
            rate
    ...
    def test_the_counter_range_is_what_the_rates_were_built_for(self):
